...
export default function SpecialOfferManager() {
	...
	useEffect(() => {
		AdminPageManager.getSectionItems("specialoffers", section).then((data) => {
			setSectionData(data);
		});
	}, [section]);
	...
	const handleFirstForm = async (event) => {
		...
		if (sec1Image) {
			let isUpload = await AdminPageManager.uploadForm("specialoffers", 1, {
				...firstSection,
				img: sec1Image,
			});
	...
	const handleSecondForm = (event) => {
		event.preventDefault();
		if (sec2Image) {
			AdminPageManager.uploadForm("specialoffers", section, {
				...secondSection,
				img: sec2Image,
			});
			...
			{section == 1 ? (
				<>
					...
					<div className="table-wrapper">
						<PrimaryTable
							tableHeader={tableHeader.section1}
							tableBody={sectionData}
							deleteCol={true}
				...
				<>
					...
					<div className="table-wrapper">
						<PrimaryTable
							tableHeader={tableHeader.section2}
							tableBody={sectionData}
							deleteCol={true}
							deleteRow={deleteTableRow}
						/>
				...
				<>
					...
					<div className="table-wrapper">
						<PrimaryTable
							tableHeader={tableHeader.section3}
							tableBody={sectionData}
							deleteCol={true}
							deleteRow={deleteTableRow}
						/>
